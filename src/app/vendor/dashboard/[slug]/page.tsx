...
import { submitBusinessVerificationDocumentAction, updateSellerMarketplaceSettingsAction } from "@/app/actions/vendor";
...
export default async function VendorDashboardPage({
  ...
  const canManage =
    ...
    vendor.managers.some((manager) => manager.userId === session.user.id);
  if (!canManage) {
    redirect("/vendor/dashboard");
  }
    ...
    <div className="-mx-3 space-y-6 pb-10 sm:mx-0">
      <section id="profile-assets" className="scroll-mt-24 overflow-hidden rounded-[2rem] border border-white/80 bg-white shadow-[0_22px_70px_rgba(72,44,43,0.08)]">
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
                ...
                <div className="max-w-3xl">
                  ...
                  <p className="mt-4 max-w-2xl text-sm leading-6 text-white/86">
                    {vendor.bio || "Add a business description so customers can understand your style, service, and the kinds of celebrations you love creating."}
                  </p>
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                <Button asChild variant="secondary" className="bg-white/92">
                  <Link href={`/onboarding?business=${vendor.slug}`}>
                    <Edit3 className="h-4 w-4" />
                    Edit Business
                  </Link>
                </Button>
                <CopyStorefrontLinkButton url={publicStorefrontUrl} />
                <Button asChild variant="secondary" className="bg-white/92">
                  <Link href={storefrontPath(vendor.slug)}>
                    <Share2 className="h-4 w-4" />
                    View Storefront
                  </Link>
                </Button>
                <Button asChild variant="secondary" className="bg-white/92">
                  <Link href={`/onboarding?business=${vendor.slug}#services`}>
        ...
        <div className="grid gap-px bg-[#eadbd8] sm:grid-cols-2 xl:grid-cols-6">
          ...
          <StatTile label="Storefront Views" value={profileViews ? String(profileViews) : "New"} />
        </div>
      </section>
      ...
      <nav className="sticky top-[88px] z-10 flex gap-2 overflow-x-auto rounded-full border border-border/70 bg-white/90 p-2 shadow-[0_14px_36px_rgba(72,44,43,0.08)] backdrop-blur">
        {["Overview", "Services", "Reviews", "Party Posts", "Portfolio", "Availability", "Credentials"].map((item) => (
          <a
            key={item}
            href={`#${slugify(item)}`}
      ...
      <section id="overview" className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
            ...
            <div>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                {specialties.length ? (
                  ...
                  <SoftChip>Add specialties in setup</SoftChip>
                )}
              </div>
            </div>
        ...
        <Panel className="p-6 sm:p-7">
          <SectionKicker icon={<Wand2 className="h-4 w-4" />} label="Business readiness" />
          <ReadinessChecklist
            items={[
              { complete: Boolean(vendor.logoUrl), label: "Business logo", href: `/onboarding?business=${vendor.slug}#profile` },
              { complete: Boolean(vendor.coverPhoto || vendor.photos.length), label: "Cover and portfolio imagery", href: `/onboarding?business=${vendor.slug}#profile` },
              { complete: Boolean(vendor.bio), label: "Business description", href: `/onboarding?business=${vendor.slug}#profile` },
              { complete: vendor.offerings.length > 0, label: "At least one service", href: `/onboarding?business=${vendor.slug}#services` },
      ...
      <section id="services" className="scroll-mt-24 space-y-4">
        ...
        {vendor.offerings.length ? (
          ...
          <EmptyState
            ...
            href={`/onboarding?business=${vendor.slug}#services`}
            action="Add service"
          />
        )}
      </section>
        ...
        {partyPosts.length ? (
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {partyPosts.map((post) => (
              <Link
                key={post.id}
                href={post.slug ? `/events/${post.slug}` : storefrontPath(vendor.slug)}
      ...
      <section id="portfolio" className="scroll-mt-24 space-y-4">
        ...
        {portfolioImages.length ? (
          ...
          <EmptyState
            ...
            href={`/onboarding?business=${vendor.slug}#profile`}
      ...
      <section id="availability" className="scroll-mt-24 grid gap-5 xl:grid-cols-[0.9fr_1.1fr]">
        <Panel className="p-6 sm:p-7">
          ...
          <div className="mt-6">
            <ConnectStripeButton businessSlug={vendor.slug} connected={vendor.stripeOnboardingComplete} />
          </div>
        </Panel>
          ...
          <div className="mt-5 rounded-[1.4rem] border border-[#eadbd8] bg-[#fbf7f5] p-4">
            ...
            {seller ? (
              <form action={updateSellerMarketplaceSettingsAction} className="mt-4 space-y-3 rounded-[1.1rem] bg-white p-3 text-sm">
                <input type="hidden" name="businessId" value={vendor.id} />
      ...
      <section id="credentials" className="scroll-mt-24 space-y-4">
        <SectionHeader
          eyebrow="Credentials & Verification"
          title="Reviewed trust signals for this storefront"
          body="Insurance, license, and permit claims only appear publicly after ShopFia reviews supporting documentation."
        />
        <div className="grid gap-4 md:grid-cols-3">
          {["INSURANCE", "LICENSE", "PERMIT"].map((type) => {
            const document = getLatestCredentialDocument(vendor.verificationDocuments, type);
            return (
              <Panel key={type} className="p-5">
                ...
                <div className="mt-3 text-xl font-semibold tracking-tight">
                  {credentialStatusLabel(document?.status)}
                </div>
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  {credentialStatusDescription(document?.status)}
                </p>
                {document?.originalName ? (
                  <div className="mt-3 truncate rounded-full bg-[#fbf7f5] px-3 py-1 text-xs text-muted-foreground">
                    {document.originalName}
                  </div>
                ) : null}
                <form action={submitBusinessVerificationDocumentAction} className="mt-4 space-y-3">
                  <input type="hidden" name="businessId" value={vendor.id} />
                  <input type="hidden" name="type" value={type} />
                  <label className="block text-xs font-medium text-muted-foreground">
                    PDF, JPG, PNG, or WebP. 8 MB max.
                    <input
                      name="document"
                      type="file"
                      accept="application/pdf,image/jpeg,image/png,image/webp"
                      className="mt-2 block w-full rounded-[1rem] border border-[#eadbd8] bg-white px-3 py-2 text-xs"
                      required
                    />
                  </label>
                  <label className="block text-xs font-medium text-muted-foreground">
                    ...
                    <input
                      name="expiresAt"
                      type="date"
                      className="mt-2 h-10 w-full rounded-full border border-[#eadbd8] bg-white px-3 text-sm"
                    />
                  </label>
                  <Button type="submit" size="sm" variant="secondary">Submit for review</Button>
                </form>
              </Panel>
            );
          })}
        ...
        <div className="absolute inset-0 select-none p-5 opacity-55 blur-[1.5px] saturate-75 sm:p-8">
          <div className="overflow-hidden rounded-[2rem] border border-white bg-white">
            ...
            <div className="grid gap-px bg-[#eadbd8] sm:grid-cols-4">
              {["Parties Served", "Reviews", "Repeat Clients", "Storefront Views"].map((item, index) => (
        ...
        <div className="absolute inset-0 grid place-items-center bg-white/58 px-5 backdrop-blur-[6px]">
          <div className="max-w-2xl rounded-[2rem] border border-white/90 bg-white/96 p-7 text-center shadow-[0_24px_80px_rgba(72,44,43,0.14)] sm:p-10">
            ...
            <p className="mx-auto mt-4 max-w-xl text-base leading-7 text-muted-foreground">
              Create a business storefront to showcase services, collect reviews, and get discovered through real parties.
            </p>
...
function getLatestCredentialDocument(
  documents: Array<{ originalName: string | null; status: string; type: string; updatedAt?: Date }>,
  type: string
) {
...
function credentialStatusLabel(status?: string) {
  ...
  if (status === "REJECTED_NEEDS_REVISION") return "Rejected or needs revision";
  if (status === "EXPIRED") return "Expired";
  return "Not submitted";
}

function credentialStatusDescription(status?: string) {
  if (status === "PENDING_REVIEW") return "ShopFia is reviewing the uploaded document before showing a public badge.";
  if (status === "VERIFIED") return "This claim can appear publicly on the storefront.";
  if (status === "REJECTED_NEEDS_REVISION") return "Upload updated documentation before this claim can be shown publicly.";
  if (status === "EXPIRED") return "Upload a current document before this claim can be shown publicly.";
  return "Upload documentation when you want ShopFia to review this credential.";
}
